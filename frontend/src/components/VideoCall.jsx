...
const VideoCall = ({ selectedChat, onClose, isReceivingCall, callerInfo }) => {
  ...
  const checkDevicePermissions = async () => {
    try {
      // Kiểm tra và yêu cầu quyền truy cập trực tiếp
      const stream = await navigator.mediaDevices.getUserMedia({ 
        video: true, 
        audio: true,
        // Thêm các constraints để tăng khả năng tương thích
        video: {
          width: { ideal: 1280 },
          height: { ideal: 720 },
          facingMode: "user"
      ...
      if (!videoTrack || !audioTrack) {
        setDeviceError('Không thể truy cập camera hoặc microphone. Vui lòng kiểm tra thiết bị.');
        return false;
      }
      
      // Dừng stream để kiểm tra
